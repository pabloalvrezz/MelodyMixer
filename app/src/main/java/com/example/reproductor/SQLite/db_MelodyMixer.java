...
public class db_MelodyMixer extends SQLiteOpenHelper {
    ...
    public void eliminarCancionFavoritos(PlayList playlist, Canciones canciones) {
        ...
        long idCancion = canciones.getId();

        String whereClause = "id_playlist = ? AND id_cancion = ?";
        String[] whereArgs = {String.valueOf(idPlaylist), String.valueOf(idCancion)};

        db.delete(tabla_PLAYLIST_CANCION.TABLE_NAME, whereClause, whereArgs);

    }

    // Método para verificar si una canción está en la tabla CANCIONES
    public boolean estaCancionEnTablaCanciones(Canciones cancion) {
        SQLiteDatabase db = this.getReadableDatabase();

        // Consulta para verificar si la canción está en la tabla CANCIONES
        String consulta = "SELECT * FROM " + tabla_CANCIONES.TABLE_NAME +
                " WHERE " + tabla_CANCIONES.ColumnasCanciones.COLUMNA_ID + " = ?";

        // Parámetros para la consulta
        String[] parametros = {String.valueOf(cancion.getId())};

        // Ejecutar la consulta
        Cursor cursor = db.rawQuery(consulta, parametros);

        // Verificar si la canción está en la tabla CANCIONES
        boolean isCancionEnTabla = cursor.moveToFirst();
    ...
    //Actualizar datos, cuando el usuario trata de cambiar sus datos.
    public void Actualizardatos(SQLiteDatabase db, String ID_Usuario, String USUARIO, String APELLIDOS, String CONTRASEÑA)
    {
        ContentValues values = new ContentValues();

        // Formar la cláusula WHERE
        String whereClause =  tabla_USUARIOS.ColumnasUsuarios.COLUMNA_ID + "=?";
        String[] whereArgs = {ID_Usuario};

        // Actualizar la columna COLUMN_NOMBRE
        values.put(tabla_USUARIOS.ColumnasUsuarios.COLUMNA_USUARIO, USUARIO);

        // Actualizar la columna "nombre_columna2"
        values.put(tabla_USUARIOS.ColumnasUsuarios.COLUMNA_APELLIDOS, APELLIDOS); // Reemplaza con el nombre real de tu segunda columna

        // Actualizar la columna "nombre_columna3"
        values.put(tabla_USUARIOS.ColumnasUsuarios.COLUMNA_CONTRASEÑA, CONTRASEÑA); // Reemplaza con el nombre real de tu tercera columna

        // Ejecutar la consulta UPDATE
        db.update(tabla_USUARIOS.TABLE_NAME, values,whereClause, whereArgs);
    }
}
